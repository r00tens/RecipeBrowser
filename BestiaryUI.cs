...
namespace RecipeBrowser
{
	...
	internal class BestiaryUI
	{
		...
		internal UIElement CreateBestiaryPanel()
		{
			...
			npcNameFilter.ValidLengthResolver = (input) =>
				npcNameFilter.CalculateMaxValidLength(
					input,
					Lang.GetNPCNameValue,
					Enumerable.Range(NPCID.NegativeIDCount + 1, NPCLoader.NPCCount - NPCID.NegativeIDCount - 1)
				);

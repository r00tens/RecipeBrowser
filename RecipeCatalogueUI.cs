	...
	internal class RecipeCatalogueUI
	{
		...
		internal UIElement CreateRecipeCataloguePanel()
		{
			...
			itemNameFilter.ValidLengthResolver = input =>
				itemNameFilter.CalculateMaxValidLength(
					input,
					recipe => recipe.createItem.Name,
					Main.recipe.Take(Recipe.numRecipes)
				);
			...
			itemDescriptionFilter.ValidLengthResolver = input =>
				itemDescriptionFilter.CalculateMaxValidLength(
					input,
					recipe => GetTooltipsAsString(recipe.createItem.ToolTip),
					Main.recipe.Take(Recipe.numRecipes)
				);
